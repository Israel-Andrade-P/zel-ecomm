...
public class GetUserLocationsTest extends LocationControllerBaseTest{

    @Test
    @DisplayName(
            """
                    WHEN: it hits /locations/user
    ...
    void greenPath() {
        ...
        when(locationService.getUserLocations()).thenReturn(locations);

        var result = mockMvc
                .get()
                .uri(BASE_URI.concat("/locations/user"))
                .exchange();
        ...
        verify(locationService).getUserLocations();
    }
}
