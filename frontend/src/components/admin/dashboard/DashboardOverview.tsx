...
const DashboardOverview = ({ title, amount, Icon, isCurrency = false }) => {
    const convertedAmount = isCurrency ? Number(amount).toFixed(2) : amount;
    ...
    return (
        <div className="xl:w-80 w-full space-y-4 text-center md:text-start px-5 py-8">
            ...
            <h1 className="font-bold text-slate-800 text-3xl">
                {isCurrency ? "$" : null}
                {isCurrency ? formatRevenue(convertedAmount) : convertedAmount}
            </h1>
        </div>
    )
}
